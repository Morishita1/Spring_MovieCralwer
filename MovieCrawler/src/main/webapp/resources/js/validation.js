var joinValidate = {
		...
		checkPw : function(memPw,memRpw) {
			var regEmpty = /\s/g; // 공백문자
			var pwReg = RegExp(/^[a-zA-Z0-9]{4,12}$/); // 유효성 체크
			
			if((memPw == "") || (memPw.length == 0)) { // null 체크
				return this.resultCode.empty_val;
			} else if(memPw.match(regEmpty)) { // 공백체크
				return this.resultCode.space_length_val;
			} else if(!pwReg.test(memPw)) {
				return this.resultCode.invalid_pw; // pw 정규식(+길이) 체크
			} else if((memRpw != "") || (memRpw.length != 0)) {
				// 비밀번호와 비밀번호 재확인 일치하는지 확인
				if(memPw == memRpw) {
		...
		checkPhone : function(phone) {
			...
			if((phone == "") || (phone.length == 0)) {
				...
				return this.resultCode.onlyNum;
	...
	$.ajax({
		...
		success: function(data) {
			if(data == 1) {
				...
				$('#inputid').next().text('이미 사용중인 아이디입니다.').css('display','block').css('color','#FF3636');
				return "overlap";
			} else {
